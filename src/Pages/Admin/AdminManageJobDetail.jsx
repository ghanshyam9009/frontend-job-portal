...
const AdminManageJobDetail = () => {
  const { jobId } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const { theme } = useTheme();
  const { user } = useAuth();
  const adminId = user?.admin_id || user?.id || user?.user_id;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setError("");
      try {
        const resolved = await adminService.getAdminPostedJobForManage(adminId, jobId);
        if (cancelled) return;
        if (!resolved) {
  ...
  const editState = job?.edit != null ? String(job.edit).trim() : "";

  const titleDisplay = (job?.job_title || "").trim() || "—";
  const companyDisplay = (job?.company_name || "").trim() || "—";
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-xs sm:text-sm ${textSecondary} truncate`}>
                {companyDisplay} · {locationDisplay}
              </p>
            </div>
          </div>
        </div>
              ...
              >
                {logoUrl ? (
                  ...
                  <span className={`text-2xl sm:text-3xl font-bold ${textSecondary}`}>
                    {getCompanyInitials(companyDisplay)}
                  </span>
                )}
              </div>
                ...
                <div>
                  ...
                  >
                    <span className="inline-flex items-center justify-center sm:justify-start gap-2">
                      ...
                      <span className="break-words">{companyDisplay}</span>
                    </span>
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              <div className={`flex items-start gap-2 ${textSecondary}`}>
                ...
                <div>
                  ...
                  <span className={`${textColor} break-all`}>
                    {jobNumericId != null ? String(jobNumericId) : "—"}
                  </span>
                </div>
              </div>
              <div className={`flex items-start gap-2 ${textSecondary}`}>
                ...
                <div>
                  ...
                  <span className={textColor}>{companyDisplay}</span>
                </div>
              </div>
              <div className={`flex items-start gap-2 ${textSecondary}`}>
                ...
                <div>
                  ...
                  <span className={`${textColor} break-words`}>{locationDisplay}</span>
                </div>
              </div>
              <div className={`flex items-start gap-2 ${textSecondary}`}>
              ...
              {(postedBy ||
                ...
                <div className={`sm:col-span-2 flex flex-wrap gap-x-4 gap-y-1 text-xs ${textSecondary}`}>
                  ...
                  {job?.admin_id != null && (
                    <span>
                      Admin ID: <span className={textColor}>{String(job.admin_id)}</span>
                    </span>
                  )}
                  {job?.employer_id != null && (
                    <span>
            ...
            <button
              ...
              onClick={() =>
                navigate(`/admin/job-applications/${jobId}`, {
                  state: {
                    jobTitle: titleDisplay,
                    companyName: companyDisplay,
                  },
                })
              }
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 shrink-0 w-full sm:w-auto"
            >
              <Users size={18} />
              View applications
            </button>
            <button
              ...
              onClick={() =>
                navigate(`/admin/edit-job/${job?.job_id || job?.id || jobId}`, {
                  state: { employer_id: job?.employer_id, admin_id: job?.admin_id },
                })
              }
              className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-blue-300 text-blue-700 bg-blue-50 dark:bg-blue-500/20 dark:text-blue-300 text-sm font-medium w-full sm:w-auto`}
            >
              <Briefcase size={16} />
              Edit job
            </button>
